...
public class Command implements SubCommand {
	...
	public void execute(Sender sender, String[] args, com.onigori.serverutility.commands.Command instance) {
		if (args.length >= 2) {
			...
			if (command != null) {
				String usage = LocalizedUtils.getLocalizedMessage(command.getUsageKey(), sender.getLocale())
				...
				String description = "Undefined.";//LocalizedUtils.getLocalizedMessage(command.getDescriptionKey(), sender.getLocale());
